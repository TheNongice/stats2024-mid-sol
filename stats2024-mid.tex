...
Mick tosses 7 coins. What is the probability that there are at least 2 heads and at least 2 tails?
...
\begin{solutionbox}
  จากโจทย์กำหนดทำให้เราทราบว่า $n(S) = 2^7 = 128$ วิธี โดยวิธีการหาจำนวนผลลัพธ์ที่สนใจสำหรับกรณีหาของที่ต้องออกลักษณะ\textbf{อย่างน้อย}\textbf{/}\textbf{อย่างมาก}
  มีวิธีการหาอยู่ 2 รูปแบบนั่นคือการหาด้วยวิธี Complement (เข้าใจได้ง่ายที่สุดและสับสนน้อยที่สุด) กับหาโดยนับกรณีที่ถูกต้องจริง ๆ (ต้องกังวลปัญหา Overcounting และการแบ่งกรณีซับซ้อน)
  โดยใน Solution นี้จะเฉลยในรูปแบบของวิธี Complement ซึ่งคำนวณได้จาก...
  \begin{align*}
    n(\text{กรณีที่สนใจ}) = n(\text{วิธีจัดกรณีไม่มีข้อจำกัด}) - \sum_{k=1}^{n(\text{กรณีที่ผิด})} n(\text{กรณีที่ผิด}_k)
  \end{align*}
  ในที่นี้มีกรณีที่ผิดกฏอยู่ 4 แบบ ซึ่งสามารถแบ่งและคำนวณได้ในลักษณะต่อไปนี้
  \begin{align*}
    \text{กรณีไม่ปรากฏ ``หัว" เลย} &= \binom{7}{0} = 1 \text{ วิธี} \\
    \text{กรณีไม่ปรากฏ ``ก้อย" เลย} &= \binom{7}{0} = 1 \text{ วิธี} \\
    \text{กรณีปรากฏ ``หัว" เพียง 1 ครั้ง} &= \binom{7}{1} = 7 \text{ วิธี} \\
    \text{กรณีปรากฏ ``ก้อย" เพียง 1 ครั้ง} &= \binom{7}{1} = 7 \text{ วิธี}
  \end{align*}

  ทำให้เราทราบได้ว่า $n(\text{กรณีที่สนใจ})\text{ หรือ } n(E) = 128-1-1-7-7 = 112$ วิธี

  เมื่อเราทราบทั้งค่าของ $n(E), n(S)$ แล้วทำให้เราสามารถหาค่าความน่าจะเป็นได้ว่า $\frac{n(E)}{n(S)} = \frac{112}{128} = \frac{7}{8}$
...
\groupctx{26--28}{A mobile game has a gacha (loot box) with a 20\% chance of getting a rare character. Furthermore, when pulling 2 consecutive rolls, if the first roll is not a rare character, then the second roll will have a chance of getting a rare character increased to 40\% (otherwise, the second roll will just have a normal rate). Fai pulls the gacha 2 times.}
 
\begin{problembox}{Problem 26}
What is the probability that her second roll is a rare character?
...
\begin{solutionbox}
  จากสิ่งที่กำหนดทำให้ทราบว่าโอกาสที่ได้จะได้ตัวละครแรร์คือ 20\% แสดงว่าโอกาสที่จะไม่ได้แรร์คือ 80\%
  หากเราสุ่มกาชา 2 ครั้งโดยที่ครั้งแรกไม่ได้แรร์ จะทำให้โอกาสได้แรร์ในการสุ่มครั้งที่ 2 เพิ่มขึ้นเป็น 40\% ทันที

  โจทย์ต้องการทราบความน่าจะเป็นเมื่อการสุ่มครั้งที่ 2 จะได้ตัวละครแรร์นั้น จะต้องพิจารณาถึงกรณีย่อยเพื่อให้คำนวณได้ดังนี้
  \begin{align*}
    \text{กรณีครั้งที่ 1 และ 2 ได้แรร์} &= 0.2 \times 0.2 = 0.04 \\
    \text{กรณีครั้งที่ 1 ไม่แรร์แต่ครั้งที่ 2 ได้แรร์} &= 0.8 \times 0.4 = 0.32
  \end{align*}
  
  $\therefore$ โอกาสที่รอบ 2 จะสุ่มแล้วได้ตัวละครแรร์คิดเป็น $0.04+0.32=0.36$ $(\text{หรือ } 36\%)$

  ตอบ $\boxed{3)}$
\end{solutionbox}

\begin{problembox}{Problem 27}
If her second roll is a rare character, what is the probability that her first roll is also a rare character?
...
\begin{solutionbox}
  \begin{flushleft}
  $\begin{aligned}
    \text{กำหนดให้ } F &:= \text{ครั้งที่ 1 ได้ตัวละครแรร์} \\
    S &:= \text{ครั้งที่ 2 ได้ตัวละครแรร์}
  \end{aligned}$
  \end{flushleft}

  จากสิ่งที่โจทย์ถามตอนนี้เราทราบ $P(S)=0.36$ แต่โจทย์ต้องการทราบค่าของ $P(F|S)$ สามารถหาได้โดยใช้ Bayes Theorem จากการคำนวณดังนี้
  \begin{align*}
      P(F|S) &= \frac{P(S|F)P(F)}{P(S)} = \frac{P(F \cap S)}{P(S)} \\
             &= \frac{0.04}{0.36} \\
             &= 0.11\overline{1} \text{ (หรือ $\frac{1}{9}$)}
  \end{align*}

  เหตุผลที่เราเลือกหยิบ $P(F \cap S) = 0.04$ ได้นั่นเพราะความหมายคือกรณีที่ทั้ง $P$ และ $S$ ได้ตัวละครแรร์พร้อมกัน
  ซึ่งมีค่าเท่ากับ $0.04$ ตามที่ได้อธิบายหลักการคำนวณไว้ใน Solution ข้อที่ 27.

  ตอบ $\boxed{4)}$
\end{solutionbox}

\begin{problembox}{Problem 28}
If her second roll is not a rare character, what is the probability that her first roll is a rare character?
...
\begin{solutionbox}
  ...
  จากสิ่งที่โจทย์ถามตอนนี้เราทราบ $P(S')=1-0.36=0.64$ แต่โจทย์ต้องการทราบค่าของ $P(F|S')$ สามารถหาได้โดยใช้ Bayes Theorem จากการคำนวณดังนี้
  \begin{align*}
      P(F|S') &= \frac{P(S'|F)P(F)}{P(S')} = \frac{P(F \cap S')}{P(S)}
  \end{align*}

  แต่ตอนนี้เรายังไม่ทราบค่าของ $P(F \cap S')$ ซึ่งมีความหมายคือการสุ่มครั้งที่ 1 ได้แรร์แต่ครั้งที่ 2 ไม่ได้แรร์ซึ่งสามารถคำนวณได้เป็น $0.2 \times 0.8 = 0.16$
  ทำให้เราสามารถหยิบมาคำนวณต่อโดยใช้หลักการของ Bayes Theorem ดังนี้
  \begin{align*}
      P(F|S') &= \frac{P(F \cap S')}{P(S)} \\
              &= \frac{0.16}{0.64} \\
              &= 0.25 \text{ (หรือ $\frac{1}{4}$)}
  \end{align*}
  ตอบ $\boxed{3)}$
\end{solutionbox}
